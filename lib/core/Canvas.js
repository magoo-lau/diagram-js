...
Canvas.prototype._init = function(config) {
  ...
  // debounce canvas.viewbox.changed events for smoother diagram interaction
  // 防抖动 
...
/////////////// markers ///////////////////////////////////
//添加标记 exp ： hover  类似 jquery.addClass removeClass toggleClass
 ...
 * Adds a marker to an element (basically a css class).
	将标记添加到一个元素(基本上一个css类)。
 *
 * Fires the element.marker.update event, making it possible to
 * integrate extension into the marker life-cycle, too.
	触发 element.marker.update 事件，组成一个完整的生命周期
 *
 * @example
...
Canvas.prototype.viewbox = function(box) {
  ...
  if (!box) {
	...
	//计算diagrams的图层的基础内容器，允许我们排除额外的组件，比如覆盖层
 ...
 * the first argument to force recomputation of the current level.
 如果第一个参数传入 false 强制重新计算当前的级别
 *
 * @param {String|Number} [newScale] the new zoom level, either a number, i.e. 0.9,
 *                                   or `fit-viewport` to adjust the size to fit the current viewport
	可以传入一个数字  或者 fit-viewport 则调整为当前视图
